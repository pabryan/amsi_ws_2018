...
#+BEGIN_defn
*Definition*: A smooth curve \(\gamma : [0, 1] \to M\) is a geodesic if
\[
\nabla_{\gamma'} \gamma' = 0.
\]
#+END_defn
...
#+BEGIN_lem
*Lemma*: Geodesics have constant speed. That is \(|\gamma'| \equiv \text{const}\).
#+END_lem

#+BEGIN_proof
*Proof*: By metric compatibility we have
\[
...
#+BEGIN_thm
*Theorem*: Suppose \(\gamma\) is length minimising. Then \(\gamma\) is a geodesic. Conversely, every geodesic is locally length minimising in the sense that for all \(t_0 \in [0, 1]\), there is an \(\epsilon > 0\) such that \(\gamma\) restricted to \([0, 1] \cap \{|t - t_0| < \epsilon\}\) is length minimising.
#+END_thm
...
#+BEGIN_proof
*Proof of the first part*: A /variation/ of \(\gamma\) is a map
\[
...
#+BEGIN_rem
*Remark*: Another way of saying this is that Euler-Lagrange equation for the length functional of curves with fixed end points \(x, y \in M\) is precisely the geodesic equation. *The critical points of the length functional are geodesics*.
#+END_rem
...
#+BEGIN_ex
*Exercise*: Show that for a vector field \(V\) along \(\gamma\) with \(V(0) = 0, V(1) = 0\), if
\[
...
#+BEGIN_ex
*Exercise*: Show that given a vector field \(S\) along \(\gamma\), there exists a variation of \(\gamma\) with variation field \(S\). /Hint/: You need to define for each \(t\), a curve \(\alpha_t(s)\) such that \(\alpha_t(0) = \gamma(t)\) and \(\alpha_t'(0) = S(t)\). This must be done is such a way that the map
\[
F(t, s) = \alpha_t(s)
\]
