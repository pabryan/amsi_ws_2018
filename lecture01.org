...
The idea here is to split the geodesic equation into a system of second order equations. We did this in coordinates in order to prove the existence and uniqueness of the geodesic equation. Here we do it intrinsically without reference to coordinates. That is, we obtain a first order system on \(TM\). The usefulness of this approach is that we may then immediately infer the smooth dependence of geodesics on the initial tangent vector (which includes smooth dependence on the base point \(x\)). As a result we conclude some important properties about the exponential map.
