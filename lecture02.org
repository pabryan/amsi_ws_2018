...
A = \nabla V.
\]
where \(V = \gamma'\) as before. That is, \(A : \Gamma(\gamma, TM) \to \Gamma(\gamma, TM)\) is a \(C^{\infty}\) linear (tensorial) map taking vector fields \(X\) along \(\gamma\) to vector fields along \(\gamma\) defined as
\[
A(X) = \nabla V (X) = \nabla_X V.
\]

Given a Jacobi field, \(J\) along \(\gamma\) we have a geodesics variations \(\varphi : [0, 1] \times (-\epsilon, \epsilon) \to M\) such that \(\varphi_{\ast} \partial_s|_{s=0} = J\) and \(\varphi(t, 0) = \gamma(t)\) so that \(\varphi_{\ast} \partial_t |_{s=0} = \gamma'\). In particular, as already used
\[
[J, \gamma'] = 0.
\]
Therefore,
...
That is, the Jacobi equation is equivalent to
\begin{cases}
\nabla_V J &= A (J) \\
\nabla_V A + A^2 + \operatorname{R}_V &= 0.
\end{cases}

The initial conditions specifying \(J(0)\) and \(J'(0)\) are now replaced by initial conditions specifying \(J(0)\) and \(A(0)\).
